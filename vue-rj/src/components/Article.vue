        ...
        <div class="foot-btn">
            <ul>
                ...
                <li class="xing"><a href="javascript:;">
                    <i><img src="http://localhost:8080/src/assets/img/xing.png" alt=""/></i>
                </a></li>
                <li class="fx"><a href="javascript:;">
                    <i><img src="http://localhost:8080/src/assets/img/fx.png" alt=""/></i>
                </a></li>
            </ul>
        </div>
    </div>
...
<script>
    export default {
        ...
        mounted(){
            this.fecthData(this.$route.query.id)
        },
        methods:{
            fecthData(id){
                let _this = this;
                this.$http.get('src/mock/article.data').then((req)=>{
                        _this.articleDetail = req.data[id]
                }).catch((err)=>{
...
<style scoped>
    .nav{width:100%; position:fixed;top:0;left:0; background:#fff; border-bottom:1px solid #e8eaec; z-index:99;}
        ...
        .nav ul li{width:0.29rem;height:0.31rem; background:url(http://localhost:8080/src/assets/img/history.png) no-repeat 0 0; background-size:100%; margin:0 0 0 0.38rem;}

        .content{max-width:6.4rem; margin:0 auto; margin-top:0.6rem; background:#f2f4f5; padding-bottom:0.85rem;}
        .content .header{ padding:0.39rem 0.28rem 0.15rem 0.28rem;}
        ...
        .say i{width:0.36rem;height:0.26rem; background:url(http://localhost:8080/src/assets/img/say.png) no-repeat 0 0; background-size:100%; float:left; margin-left:0.7rem; margin-top:0.13rem;}
        ...
        .zan i{width:0.36rem;height:0.28rem; background:url(http://localhost:8080/src/assets/img/zan1.png) no-repeat 0 0; background-size:100%; float:left; margin-left:0.54rem; margin-top:0.13rem;}
